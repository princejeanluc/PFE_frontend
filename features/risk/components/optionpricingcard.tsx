...
import { useCryptoList } from "@/features/market/hooks/market";
import { useOptionPricing } from "@/features/risk/hooks/risk";
...
export default function OptionPricingCard() {
  ...
  const [riskFree, setRiskFree] = React.useState<number | "">("");
  ...
  const canSubmit =
    ...
    ((useDates && currentDate && maturityDate) ||
      (!useDates && typeof horizonHours === "number" && horizonHours > 0));
  ...
  const setATM = () => {
    if (spot != null) setStrike(Number(spot.toFixed(2)));
  };
  ...
  const tMeta = React.useMemo(() => {
    ...
    const tYears = diffH / (24 * 365);
    return { hours: diffH, years: tYears };
  }, [useDates, currentDate, maturityDate]);

  const computeLoad =
    (useDates && tMeta ? Math.round(tMeta.hours) : typeof horizonHours === "number" ? horizonHours : 0) * nSims;
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2 text-xs text-muted-foreground">
            ...
            <span className="hidden md:inline">
              Charge estimée: <strong>{computeLoad.toLocaleString()}</strong>
            </span>
          </div>
        </div>
      </CardHeader>
      ...
      <CardContent className="space-y-4">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
          ...
          <div className="space-y-2">
            ...
            {spot != null && <div className="text-xs text-muted-foreground">Spot estimé: ${spot.toFixed(2)}</div>}
          </div>
          ...
          <div className="space-y-2">
            ...
            <div className="flex gap-2">
              ...
              <Button
                size="sm"
                variant="outline"
                onClick={() => strike && setStrike(Number(strike) * 0.9)}
                disabled={!strike || pricing.isPending}
              >
                -10%
              </Button>
              <Button
                size="sm"
                variant="outline"
                onClick={() => strike && setStrike(Number(strike) * 1.1)}
                disabled={!strike || pricing.isPending}
              >
                +10%
              </Button>
            ...
            {useDates ? (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                ...
                <div className="md:col-span-2 text-xs text-muted-foreground">
                  {tMeta ? (
                    <>
                      T ≈ {tMeta.hours.toFixed(0)}h ({tMeta.years.toFixed(4)} an(s))
                    </>
                  ) : (
              ...
              <div className="space-y-1">
                ...
                <div className="flex gap-2 pt-1">
                  <Button size="sm" variant="secondary" onClick={() => setPresetH(24)} disabled={pricing.isPending}>
                    1j
                  </Button>
                  <Button size="sm" variant="secondary" onClick={() => setPresetH(72)} disabled={pricing.isPending}>
                    3j
                  </Button>
                  <Button size="sm" variant="secondary" onClick={() => setPresetH(168)} disabled={pricing.isPending}>
                    7j
                  </Button>
                </div>
              </div>
        ...
        <div className="flex flex-wrap gap-2">
          ...
          {typeof (pricing as any).cancel === "function" && pricing.isPending && (
            <Button variant="ghost" onClick={() => (pricing as any).cancel()}>
          ...
          <div className="space-y-3">
            ...
            {showDetails && (
              <div className="rounded-md border p-3 text-xs text-muted-foreground space-y-1">
                <div>Modèle : {String(pricing.data.diagnostics?.model_used ?? "—")}</div>
              </div>
            )}
          </div>
        )}
