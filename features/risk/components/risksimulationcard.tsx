...
import { useRiskSimulation } from "@/features/risk/hooks/risk";
import { Button } from "@/components/ui/button";
import { Loader2, XCircle } from "lucide-react";
import { buildPriceRows, buildVolRows } from "@/features/risk/domain/simulation";
...
export default function RiskSimulatorCard() {
  ...
  const [params, setParams] = React.useState<{ symbol: string; horizon: number; nSims: number } | null>(
    null
  );
  ...
  const { data, isError, isFetching, error, cancel } = useRiskSimulation(
    params?.symbol,
    params?.horizon,
    params?.nSims,
    {
  ...
  const computeLoad = formHorizon * formNSims;
            ...
            <div className="h-72">
              {!data && !isFetching ? (
                <div className="h-full flex items-center justify-center text-sm text-muted-foreground">
                  ...
                  <strong className="ml-1">Lancer la simulation</strong>.
                </div>
              ) : priceRows.length === 0 ? (
