...
function ListPortFolioComponent() {
  ...
  const pageSize = 12;
  const { data: dataPortfolios, isLoading, refetch, isFetching } = usePortfolios({
    page,
    pageSize,
  ...
  const onApplyFilters = () => {
    setApplied({ start: pendingStart, end: pendingEnd });
    setPage(1);
  };
        ...
        <div className="grid gap-4 md:h-72 [grid-template-columns:repeat(auto-fit,minmax(340px,1fr))] overflow-y-scroll">
          {isLoading || isFetching
            ? Array.from({ length: 8 }).map((_, i) => (
                <Skeleton key={i} className="h-40 w-full rounded-md bg-gray-200" />
              ))
            : dataPortfolios?.results.map((p: any) => (
                <PortefolioCard key={p.id} portfolio={p} onDeleted={() => refetch()} />
              ))}
          {!isLoading && dataPortfolios?.results?.length === 0 && (
            <div className="col-span-full text-sm text-gray-500">
              Aucun portefeuille trouvé avec ces critères.
            </div>
          )}
        </div>
      </div>
