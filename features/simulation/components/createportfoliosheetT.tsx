...
import { useCreateHoldings, useCreatePortfolio } from "@/features/simulation/hooks/simulation";
import { getCryptoList } from "@/features/market/api/market";
...
export default function CreatePortfolioSheet({ onSuccess }: { onSuccess: () => void }) {
  ...
  useEffect(() => {
    getCryptoList().then((d) => setAllCryptos(d.results));
  }, []);

  const totalAlloc = useMemo(
    () => holdings.reduce((s, h) => s + (Number(h.allocation_percentage) || 0), 0),
    [holdings]
  );
  ...
  const validStep1 = useMemo(() => {
    ...
    const datesOk =
      form.holding_start &&
      form.holding_end &&
      new Date(form.holding_end) >= new Date(form.holding_start);
  ...
  const distributeEqually = () => {
    ...
    const arr = selectedCryptos.map((c, i) => ({
      crypto: c.id,
      allocation_percentage: i === selectedCryptos.length - 1 ? last : eq,
    }));
    setHoldings(arr);
  };
  ...
  const handleCreatePortfolio = async () => {
    if (!validStep1) {
      toast.error("Vérifie le nom, budget, dates et sélection de cryptos.");
      return;
    }
    setSubmitting(true);
    try {
      ...
      if (form.allocation_type === "manual") {
        ...
        const payload = selectedCryptos.map((c) => ({
  ...
  const handleAddHoldings = async () => {
    ...
    if (Math.round(totalAlloc) !== 100) {
      toast.error(`La somme doit faire 100%. Actuellement: ${totalAlloc.toFixed(2)}%`);
      return;
    }
    setSubmitting(true);
    try {
    ...
    >
      ...
      <DialogContent className="p-6 max-w-2xl">
        ...
        <div className="mt-2 mb-4 flex items-center gap-2 text-xs">
          <span className={`px-2 py-1 rounded ${step === 1 ? "bg-primary text-white" : "bg-gray-100"}`}>
            1. Infos
          </span>
          <span className="text-gray-300">→</span>
          <span className={`px-2 py-1 rounded ${step === 2 ? "bg-primary text-white" : "bg-gray-100"}`}>
            2. Répartition
          </span>
        </div>
        ...
        {step === 1 && (
          <div className="flex flex-col gap-3">
            ...
            <Select value={form.allocation_type} onValueChange={(val) => setForm({ ...form, allocation_type: val })}>
              ...
              <SelectValue placeholder="Methode d&apos;allocation" />
              </SelectTrigger>
              <SelectContent>
          ...
          <div className="flex flex-col gap-4">
            <div className="flex items-center justify-between">
              ...
              <div className="text-xs text-gray-600">
                Somme :{" "}
                <strong className={Math.round(totalAlloc) === 100 ? "text-green-600" : "text-red-600"}>
            ...
            <div className="flex gap-2">
              <Button size="sm" variant="secondary" onClick={distributeEqually}>
