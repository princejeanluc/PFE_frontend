...
import { buildReturnSeries, intervals } from "@/features/simulation/domain/returns";

const ChartReturns = ({ performances }: { performances: any[] }) => {
  const [selectedInterval, setSelectedInterval] = useState("3M");

  const { sorted, change } = buildReturnSeries(performances, selectedInterval);
          ...
          {intervals.map((interval) => (
            <button
              ...
              className={`text-xs px-2 py-1 rounded ${
                selectedInterval === interval.label ? "bg-purple-500 text-white" : "text-gray-400"
              }`}
