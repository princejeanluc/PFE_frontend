...
import { useCryptoChartData, useCryptoList } from "@/features/market/hooks/market";
...
} from "@/features/market/domain/chart";
...
export function ChartAreaInteractive() {
  ...
  const debouncedSetSymbol = React.useMemo(
    () => debounce((v: string) => setSelectedSymbol(v), 300),
    []
  );
  React.useEffect(() => () => debouncedSetSymbol.cancel(), [debouncedSetSymbol]);

  const chartData = React.useMemo(() => mapHistoryToChartData(data?.history), [data]);
  const stats = React.useMemo(() => computePriceStats(chartData), [chartData]);
  const options = React.useMemo(() => filterCryptoOptions(cryptoList, query), [cryptoList, query]);
              ...
              <ChartContainer config={chartConfig}>
                <AreaChart data={chartData} margin={{ top: 8, right: 16, left: 0, bottom: 0 }}>
                  ...
                  <XAxis
                    ...
                    tickMargin={8}
                    minTickGap={20}
                    tickFormatter={(value) => formatTickByRange(value, timeRange)}
                  />
