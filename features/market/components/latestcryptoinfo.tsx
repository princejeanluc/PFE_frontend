...
import { useLatestCryptoInfo } from "@/features/market/hooks/market";
import Titlebar from "@/shared/components/titlebar";
import { Skeleton } from "@/components/ui/skeleton";
import { Tooltip, TooltipContent, TooltipTrigger } from "@/components/ui/tooltip";
import { pickPred, pctFromLogRet100, profitFromLogRet100, Pred } from "@/features/market/domain/predictions";
...
export default function LatestCryptoInfoComponent() {
  ...
  const pageSize = 10;
  const { data: dataCryptoLatest, isLoading: isLoadingDataCryptoLatest } = useLatestCryptoInfo({
    page,
    pageSize,
    search,
  });
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead className="font-medium text-sm text-primary text-center">
                    Profit attendu (1h){" "}
                    <span className="text-[10px] text-gray-500">(XGBoost, 1 unité)</span>
                  </TableHead>
                </TableRow>
              </TableHeader>
              ...
              <TableBody>
                ...
                {!isLoadingDataCryptoLatest &&
                  dataCryptoLatest?.results?.map((c: any) => {
                    ...
                    const gruDir = var1hPct == null ? null : Math.sign(var1hPct);
                    const xgbDir =
                      xgb?.predicted_price != null && price != null
                        ? Math.sign(xgb.predicted_price - price)
                        : xgb?.predicted_log_return == null
                        ? null
                        : Math.sign(xgb.predicted_log_return);
                    const disagree =
                      xgbDir != null && gruDir != null && xgbDir !== 0 && gruDir !== 0
                        ? xgbDir !== gruDir
                        : false;
                    const predicted_date =
                      new Date(c.latest_predictions[0]?.predicted_date).toLocaleString() || "NA";
                    ...
                    return (
                      <TableRow key={c.id}>
                        ...
                        <TableCell className="text-center text-xs">{nfPct(ret24)}</TableCell>
                        ...
                        <TableCell className="text-center text-xs">
                          {c.latest_info?.circulating_supply != null
                            ? nfInt.format(c.latest_info.circulating_supply)
                            : "—"}
                        </TableCell>
