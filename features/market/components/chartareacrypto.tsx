    ...
    <Card className="pt-0">
      <CardHeader className="flex items-center gap-2 space-y-0 border-b py-5 sm:flex-row">
        <div className="grid flex-1 gap-1">
          <CardTitle>
            <span className="font-bold text-2xl text-foreground">${latest.toLocaleString("en-US")}</span>
          </CardTitle>
          <CardDescription>
            <span className="flex items-center">
              <span className="flex justify-start font-bold text-lg" style={{ color: "oklch(0.6 0.118 184.704)" }}>
      ...
      <CardContent className="px-2 pt-4 sm:px-6 sm:pt-6">
        <ChartContainer config={chartConfig} className="aspect-auto h-[250px] w-full">
          <AreaChart data={filteredData} margin={{ top: 10, right: 30, left: 0, bottom: 0 }}>
            ...
            />
            <Area dataKey="price" type="natural" fill="url(#fillPrice)" stroke="var(--color-desktop)" strokeWidth={2} dot={false} isAnimationActive={true} />
